...
window.addVirtualMinion = function(response, command) {
...
window.isDocumentationOutput = function(response) {
...
window.reduceDocumentationOutput = function(response, visualKey, filterKey) {
...
window.addDocumentationOutput = function(outputContainer, response) {
...
window.addNormalOutput = function(outputContainer, response) {
...
window.addErrorOutput = function(outputContainer, response) {
...
window._addOutput = function(outputContainer, response, command) {

  // remove old content
  outputContainer.innerHTML = "";

  if(typeof response === "string") {
    window.addErrorOutput(outputContainer, response);
    return;
  }

  // reformat runner/wheel output into regular output
  response = window.addVirtualMinion(response, command);

  // it might be documentation
  let isDocumentationOutput = window.isDocumentationOutput(response);
  if(isDocumentationOutput) {
    command = command.trim().replace(/^[a-z.]* */i, "");
    window.reduceDocumentationOutput(response, command, command);
    window.addDocumentationOutput(outputContainer, response);
    return;
  }

  // nothing special? the it is normal output
  window.addNormalOutput(outputContainer, response);
}
