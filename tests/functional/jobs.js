...
/* eslint-enable func-names */
  ...
  describe("Jobs List", () => {
    ...
    it("should display jobs page with content", async () => {
      await page.goto(url + "#jobs");

      await page.waitForFunction(() => {
        const jobsPage = document.querySelector("#page-jobs");
        return jobsPage && jobsPage.style.display !== "none";
      });

      const pageVisible = await page.evaluate(() => {
        const page = document.querySelector("#page-jobs");
        return page !== null;
      });

      assert.isTrue(pageVisible, "jobs page should be visible");
    });

    it("should display jobs table or list", async () => {
      await page.goto(url + "#jobs");
      await page.waitForFunction(() => document.location.href.includes("#jobs"), { timeout: TIMEOUT });

      const hasTable = await page.evaluate(() => {
        const table = document.querySelector("#jobs, [class*='job'], table");
        return table !== null;
      });

      assert.isTrue(hasTable, "should have jobs table or list");
    });

    it("should have job table columns", async () => {
      await page.goto(url + "#jobs");
      await page.waitForFunction(() => {
      ...
      assert.isTrue(hasColumns, "should have table columns or headers");
    });
  ...
  describe("Job Details", () => {
    ...
    it("should display job summary panel on jobs page", async () => {
      await page.goto(url + "#jobs");
      await page.waitForFunction(() => {
        const panel = document.querySelector("#jobs-summary, [class*='summary'], [class*='panel']");
        return panel !== null;
      }, { timeout: TIMEOUT });
      ...
      assert.isTrue(hasSummary, "jobs page should have summary panel or content");
    });
  ...
  describe("Job Status", () => {
    ...
    it("should display job status information", async () => {
      await page.goto(url + "#jobs");
      await page.waitForFunction(() => {
        const pageEl = document.querySelector("#page-jobs");
        return pageEl !== null && pageEl.textContent.length > 0;
      }, { timeout: TIMEOUT });

      const hasStatusInfo = await page.evaluate(() => {
        const page = document.querySelector("#page-jobs");
        return page !== null && page.textContent.length > 0;
      });

      assert.isTrue(hasStatusInfo, "should display job status information");
    });
