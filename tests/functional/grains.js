...
/* eslint-enable func-names */
  ...
  it("should navigate to grains page", async () => {
    await page.goto(url + "#grains");
  ...
  it("should display grains page with content", async () => {
    await page.goto(url + "#grains");

    await page.waitForFunction(() => {
      const grainsPage = document.querySelector("#page-grains");
      return grainsPage && grainsPage.style.display !== "none";
    });

    const pageVisible = await page.evaluate(() => {
      const page = document.querySelector("#page-grains");
      return page !== null;
    });

    assert.isTrue(pageVisible, "grains page should be visible");
  });

  it("should display grains table with minion data", async () => {
    await page.goto(url + "#grains");
    await page.waitForFunction(() => document.location.href.includes("#grains"), { timeout: TIMEOUT });

    const hasContent = await page.evaluate(() => {
      // Check for grains table or any data display
      const table = document.querySelector("#grains, [class*='grain'], table");
      return table !== null;
    });

    assert.isTrue(hasContent, "grains page should display some content");
  });
