...
/* eslint-enable func-names */
  ...
  it("should display stats panel on options page", async () => {
    await page.goto(url + "#options");
    await page.waitForFunction(() => document.location.href.includes("#options"), { timeout: TIMEOUT });

    const hasStatsPanel = await page.evaluate(() => {
      const statsPanel = document.querySelector("#stats-panel");
      return statsPanel !== null;
    });

    assert.isTrue(hasStatsPanel, "stats panel should be displayed on options page");
  });

  it("should display stats content", async () => {
    await page.goto(url + "#options");
    await page.waitForFunction(() => document.location.href.includes("#options"), { timeout: TIMEOUT });

    const hasContent = await page.evaluate(() => {
      const statsPanel = document.querySelector("#stats-panel");
      return statsPanel && statsPanel.textContent.length > 0;
    });

    assert.isTrue(hasContent, "stats panel should have content");
  });

  it("should not display stats panel in popup mode", async () => {
    await page.goto(url + "#options?popup=true");
    await page.waitForFunction(() => document.location.href.includes("#options"), { timeout: TIMEOUT });
    ...
    assert.isTrue(hasStatsPanel, "stats panel should not be displayed in popup mode");
  });
