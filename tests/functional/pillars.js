...
describe("Pillars Page", function () {
/* eslint-enable func-names */
  ...
  it("should navigate to pillars page", async () => {
    await page.goto(url + "#pillars");
  ...
  it("should display pillars page with content", async () => {
    await page.goto(url + "#pillars");

    await page.waitForFunction(() => {
      const pillarsPage = document.querySelector("#page-pillars");
      return pillarsPage && pillarsPage.style.display !== "none";
    }, { timeout: TIMEOUT });

    const pageVisible = await page.evaluate(() => {
      const page = document.querySelector("#page-pillars");
      return page !== null;
    });

    assert.isTrue(pageVisible, "pillars page should be visible");
  });

  it("should display pillars table with minion data", async () => {
    await page.goto(url + "#pillars");
    await page.waitForFunction(() => {
      const table = document.querySelector("#pillars, [class*='pillar'], table");
      return table !== null;
    }, { timeout: TIMEOUT });
    ...
    assert.isTrue(hasContent, "pillars page should display some content");
  });
