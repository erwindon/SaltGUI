...
/* eslint-enable func-names */
  ...
  it("should navigate to mine page", async () => {
    await page.goto(url + "#mine");
    await page.waitForFunction(() => document.location.href.includes("#mine"), { timeout: TIMEOUT });

    const href = await page.evaluate(() => document.location.href);
    assert.include(href, "mine");
  });

  it("should display mine page with content", async () => {
    await page.goto(url + "#mine");

    await page.waitForFunction(() => {
      const minePage = document.querySelector("#page-mine");
      return minePage && minePage.style.display !== "none";
    }, { timeout: TIMEOUT });
    ...
    assert.isTrue(pageVisible, "mine page should be visible");
  });

  it("should display mine table with minion data", async () => {
    await page.goto(url + "#mine");
    await page.waitForFunction(() => {
      const table = document.querySelector("#mine, [class*='mine'], table");
      return table !== null;
    }, { timeout: TIMEOUT });
    ...
    assert.isTrue(hasContent, "mine page should display some content");
  });
